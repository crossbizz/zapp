...
export function ReleasesView({ projectId, releaseId }: { readonly projectId: string; readonly releaseId?: string }): ReactElement {
  ...
  return <main style={{ fontFamily: 'system-ui', margin: '0 auto', maxWidth: 960, padding: 32 }}><Link href={`/projects/${projectId}/releases`}>← Release history</Link><h1>Release {releaseId}</h1><p aria-live="polite">{status}</p>{detail === undefined ? null : <><p>Status: <strong>{detail.release.status}</strong> · commit <code>{detail.release.commitSha}</code></p><p>Readiness: <strong>{detail.readiness.state}</strong></p><ul>{detail.readiness.findings.map((finding) => <li key={finding.id}>{finding.severity}: {finding.title} — {finding.detail}</li>)}</ul>{canMutate ? <div style={{ display: 'flex', gap: 8 }}><button disabled={detail.readiness.state === 'blocked' || detail.release.status === 'approved'} onClick={() => { void mutate((client) => client.approveRelease(releaseId), 'Release approved.'); }} type="button">Approve release</button><button disabled={detail.release.status !== 'approved'} onClick={() => { void mutate((client) => client.deployRelease(releaseId), 'Deployment started.'); }} type="button">Deploy</button><button onClick={() => { void mutate((client) => client.forkRelease(releaseId), 'Repair branch created.'); }} type="button">Fork to repair</button></div> : null}</>}
