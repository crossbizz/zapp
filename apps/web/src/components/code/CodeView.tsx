...
export function CodeView({
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ensureProjectWorkspace(client, { branchId, projectId, signal: controller.signal }),
      client.getMe(controller.signal),
    ])
      .then(async ([session, me]) => {
  ...
  }, [branchId, organizationId, projectId, view]);
  const open = async (path: string): Promise<void> => {
    if (workspaceId === undefined) return;
    try {
      const loaded = await client.readWorkspaceFile(workspaceId, path);
      setFile(loaded);
      setContent(decodeBase64(loaded.dataBase64));
      setEditing(false);
      setStatus('');
    } catch {
      setStatus('The file could not be opened.');
    }
  };
  const openDirectory = async (path: string): Promise<void> => {
    ...
    try {
      ...
      setStatus('The folder could not be opened.');
    }
  };
  const save = async (): Promise<void> => {
    if (workspaceId === undefined || file === undefined) return;
    try {
      const saved = await client.editWorkspaceFile(workspaceId, {
        path: file.path,
        dataBase64: encodeBase64(content),
        expectedCompareToken: file.compareToken,
      });
      setFile({
        ...file,
        dataBase64: encodeBase64(content),
      ...
      setEditing(false);
      setStatus(`Saved in ${saved.commitRef}`);
    } catch {
      setStatus('The file changed or could not be saved. Reload and retry.');
    }
  };
  return (
    <section
