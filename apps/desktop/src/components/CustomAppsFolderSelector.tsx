...
export function CustomAppsFolderSelector() {
  ...
  const handleResetToDefault = async () => {
    try {
      ...
      await fetchCustomAppsFolder();
      showSuccess("Zapp apps folder reset successfully");
    } catch (error: any) {
      showError(`Failed to reset Zapp Apps folder path: ${error.message}`);
    }
  };
  ...
  const fetchCustomAppsFolder = async () => {
    try {
      ...
      showError(`Failed to fetch Zapp apps folder path: ${error.message}`);
    }
  };
    ...
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        <div className="text-sm text-gray-500 dark:text-gray-400">
          <p>
            {isPathAvailable
              ? "This is the top-level folder that Zapp will store new applications in."
