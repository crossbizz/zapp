...
4. durably records the non-secret ephemeral username before Forgejo user creation, records a second receipt after creation and before any collaborator grant, then grants access, re-reads and matches the immutable repository ID, and uses that repository-bound credential for the mirror push and every remote-ref read. A replacement at the same path cannot inherit the grant;
5. records `push-started`, mirror-pushes every ref from the bundle (including tags and non-branch refs), records `push-complete`, compares every non-null expected branch SHA with the `branches` table, and prepares exact branch/ref evidence;
6. revokes the ephemeral restore identity, publishes an append-only verification fence bound to the intent, backup, immutable target, and result, cleans every created credential generation, records `verified`, and removes only local temporary bundle, mirror, and askpass files.
...
Credential cleanup is also append-only. A replay never treats a DELETE 404 as terminal while a creator has only an allocation receipt: it fails closed through the username expiry plus the maximum bounded Forgejo request window, because that creator may still have `POST /admin/users` in flight. Once the created receipt exists, deletion is safe because that creator cannot recreate the identity; a crash before creation is recoverable after that bounded window. Credential allocation checks the durable verification fence both before and after its conditional receipt write. An allocation accepted before the fence is included in terminal cleanup; one that loses the race is released and refused before Forgejo creation. Verification is recorded only after every created allocation has a matching release receipt. If cleanup or the process fails after fencing, replay finishes cleanup and promotes the fence's immutable result without issuing another credential. These receipts contain the username, expiry, immutable target binding, operation digest, and non-secret verification evidence only—never the generated password or token.
