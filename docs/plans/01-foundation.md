...
- 2026-08-12 M1-GATE-16 done — the pre-push matrix exposed a source-scan TOCTOU with an ephemeral typecheck directory, an SSE/Chrome teardown ordering wait, and the production-composition entrypoint exceeding its unit-size deadline under full contention; the import boundary now scans only committed sources, stream cancellation precedes Chrome close inside an evidenced 30-second hook bound, and the entrypoint proof has a bounded 15-second budget.
