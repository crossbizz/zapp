...
- 2026-08-13 V-3 BLOCKED: Fresh route, browser, autonomous, and deployment runs closed E8 and strengthened E1/E2 without overclaiming their disconnected evidence, moving the matrix to 16 verified, 5 candidates, 1 V-2-blocked, and 0 failed; one-session journey, cross-client sync, real Stytch/GitHub/database-provider, and repeat-change evidence remains unavailable.
- 2026-08-13 V-3 BLOCKED: Reverified E8 through Mission Control itself for structured progress plus pause, resume, redirect, and cancel, with exact manifest commands, content-addressed raw output, timestamp, and committed-baseline/source binding; V-3 remains unchecked on E1/E2, live providers, and V-2.
- 2026-08-13 V-3 BLOCKED: Closed E1 with one signed-in browser journey from the initial prompt through clarification, build, authenticated preview, iteration, readiness, and deployment inside one builder route, bound to exact content-addressed output at a committed baseline; the matrix is 17 verified, 4 candidates, 1 V-2-blocked, and 0 failed.
- 2026-08-13 V-3 BLOCKED: Corrected the E1 overclaim after review: its browser journey scripts backend lifecycle responses and therefore complements, but cannot join, the separate autonomous/deployment workflow evidence. E1 is candidate again; the matrix is 16 verified, 5 candidates, 1 V-2-blocked, and 0 failed.
...
#### M1-GATE-17-FIX-3 — Recover interrupted builder sessions and explain terminal failures

**Files:** `services/model-gateway/src/usage-client.ts`, `services/model-gateway/test/budget.test.ts`, `services/orchestrator-worker/src/runtime/model-gateway-client.ts`, `services/orchestrator-worker/src/session/loop.ts`, `services/orchestrator-worker/src/session/transcript.ts`, `services/orchestrator-worker/src/workflows/run.ts`, `services/orchestrator-worker/test/model-gateway-client.test.ts`, `services/orchestrator-worker/test/session.test.ts`, `services/orchestrator-worker/test/integration/signals.test.ts`, `apps/web/src/components/conversation/Thread.tsx`, `apps/web/src/components/preview/PreviewFrame.tsx`, `apps/web/src/components/run-outcome.ts`, `apps/web/test/run-outcome.test.ts`, `apps/web/e2e/conversation.spec.ts`, `apps/web/package.json`, `docs/plans/00-master-plan.md`, `tasks/todo.md`

- [x] **Step 1:** Preserve the signed-in failure where a durable builder reached turn eight, then one model-gateway HTTP 404 was collapsed into terminal `gateway_stream_failed`, while the equal turn ordinal suppressed its only visible explanation.
- [x] **Step 2:** Add RED regressions proving transient gateway availability responses retain the in-flight completion for Temporal activity retry, JSONB key ordering cannot invalidate its structural fingerprint, terminal builder results publish one structured failed run outcome, and failed outcomes never project as successful completion in conversation or preview state.
- [x] **Step 3:** Minimally classify bounded gateway availability statuses as retryable, publish the redacted terminal summary and code, and render a compact actionable failure notice without treating it as preview-ready completion.
- [x] **Step 4:** Run focused GREEN, touched-package test/lint/typecheck/build, one signed-in prompt-to-preview/files/follow-up browser acceptance, and the complete repository gate.
- [x] **Step 5:** Check the fix task and commit `fix(local): recover interrupted builder sessions`; `M1-GATE-17` remains open for its required Modal evidence.

#### M1-GATE-17-FIX-4 — Reject false-ready local service processes

**Files:** `scripts/local/supervisor.mjs`, `scripts/local/process.mjs`, `scripts/local.test.mjs`, `docs/plans/00-master-plan.md`, `tasks/todo.md`

- [x] **Step 1:** Preserve the local failure where a `tsx watch` wrapper survived after its HTTP server child exited, allowing a stale listener to satisfy readiness while the canonical sandbox service was unavailable.
- [x] **Step 2:** Recheck each application port immediately before launch and run built backend services plus the run worker as one-shot processes; retain web hot reload.
- [x] **Step 3:** Stop complete detached process groups even when their package wrapper has already exited, with a bounded `SIGTERM` grace period and `SIGKILL` fallback.
- [x] **Step 4:** Run focused RED/GREEN, prove repeated live health and complete shutdown cleanup, run the cold repository gate, then check this fix task and commit `fix(local): reject false-ready service processes`; `M1-GATE-17` remains open for its required Modal evidence.

#### M1-GATE-17-FIX-5 — Complete repeated Ctrl-C shutdown

**Files:** `scripts/local/supervisor.mjs`, `scripts/local.test.mjs`, `docs/plans/00-master-plan.md`, `tasks/todo.md`

- [x] **Step 1:** Preserve the local failure where a second Ctrl-C during asynchronous reverse-order shutdown invokes Node's default signal behavior, terminates the supervisor early, and leaves the first application process groups listening on ports 4100, 4400, and 4500.
- [x] **Step 2:** Keep both signal handlers installed until the one idempotent shutdown promise settles, then remove them in the existing final cleanup boundary.
- [x] **Step 3:** Run the focused RED/GREEN and complete supervisor tests, prove a live double-Ctrl-C exits zero and releases every application port, run `pnpm verify`, then check this fix task and commit `fix(local): complete repeated Ctrl-C shutdown`; `M1-GATE-17` remains open for its required Modal evidence.
...
- 2026-08-16 M1-GATE-17-FIX-3 done — Canonicalized durable completion fingerprints across PostgreSQL JSONB retries, retried bounded gateway outages, exposed truthful terminal failures, suppressed terminal reconnect noise, and proved a signed-in test-first follow-up through real preview and source files; M1-GATE-17 remains open for its required Modal evidence.
- 2026-08-16 M1-GATE-17-FIX-4 done — Replaced watch wrappers with one-shot backend and worker processes, rechecked ports at every launch boundary, and bounded complete process-group shutdown; focused tests passed 17/17, repeated live health and port cleanup were green, and `pnpm verify:cold` passed; M1-GATE-17 remains open for its required Modal evidence.
- 2026-08-17 M1-GATE-17-FIX-5 done — Kept SIGINT and SIGTERM handlers active through the existing idempotent shutdown promise so repeated Ctrl-C cannot terminate cleanup early; focused regression passed 1/1, the supervisor suite passed 18/18, a live double-Ctrl-C exited zero with ports 3000/4000/4100/4400/4500 released, and `pnpm verify` exited zero; M1-GATE-17 remains open for its required Modal evidence.
...
- 2026-08-13 WEB-14-FIX-2 done — Replaced that scripted lifecycle with one real local Temporal composition: user signals now causally advance the production autonomous workflow, preview runs the public bootstrap/exchange/redemption path, candidate readiness and approval obey the release state machine, and deployment executes all eight production workflow stages; V-3 evidence remains a separate content-addressed baseline step.
- 2026-08-13 MAC-8-FIX-2 done — Bound web and desktop builder projections to one typed event stream in a focused integration test; deterministic state parity is green, but E2 remains candidate until a packaged macOS journey supplies cross-client runtime evidence.
- 2026-08-13 V-3 BLOCKED — Promoted E1 only after one signed-in browser session drove the production autonomous Temporal workflow, authenticated preview bootstrap/session redemption, iteration-dependent task, real readiness policy, public release approval, and eight-stage deployment workflow; three exact commands are bound to baseline `da42a61` and content-addressed raw logs, moving the matrix to 17 verified, 4 candidates, 1 V-2-blocked, and 0 failed.
- 2026-08-13 V-3 BLOCKED — The one final Stytch provider gate used the configured test-project credential and passed 5/5 (three PostgreSQL login lifecycle checks plus two provider-authenticated rejection probes); E3 remains candidate because this proves the shared auth boundary, not one real user completing sign-in on both packaged macOS and web clients.
