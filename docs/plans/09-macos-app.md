...
**Files:** Modify: `apps/desktop/package.json`, `apps/desktop/e2e-tests/fixtures/engine/local-agent/*`, `apps/desktop/src/testing/*`, `apps/desktop/testing/fake-http-mcp-server.mjs`, `apps/desktop/src/ipc/handlers/{__tests__/*,chat_mode_resolution*,chat_stream_handlers.ts,chat_turn_acceptance.ts,settings_handlers.ts,app_handlers.ts,compaction/*}`, `apps/desktop/src/ipc/utils/{cloud_sandbox_provider*,llm_engine_provider.ts,ollama_provider.ts}`, `apps/desktop/src/{components/chat/ChatInput.tsx,lib/schemas.ts,zapp/runtime/*}`, `apps/desktop/vitest.config.ts`, `tasks/todo.md`, this plan. Delete only obsolete provider-private/tag-processor fixtures superseded by the contained runtime and retained public/E2E coverage.
**Effort:** L

- [x] RED: inventory each remaining hybrid failure serially and classify it as current public/local-runtime behavior, an obsolete removed provider-private behavior, or a test-runner resource-isolation failure.
- [x] GREEN: retain product-valid flows through exact typed contained tools and public APIs; replace provider transport dumps with strict captured `CompleteRequest` assertions; delete or rewrite obsolete Pro/tag-processor expectations without weakening current product guarantees.
- [x] GREEN: honor typed fixture delay/usage with abortable waits, scope fixture turns to one user operation, and serialize only the resource-owning integration project if concurrent fake services remain unstable.
- [x] Verify: complete desktop `test:unit`, desktop lint/typecheck, four Playwright lists, and all focused migrated clusters are green with no skipped replacement coverage.
- [x] Commit: `test(desktop): reconcile hybrid runtime coverage`
...
### Task MAC-8-FIX-2 [M6]: Cross-client builder projection contract

**Files:** Create: `apps/desktop/src/zapp/builder-cloud/cross-client-projection.integration.test.tsx`; Modify: `tasks/todo.md`, `docs/plans/{00-master-plan,09-macos-app}.md`

- [x] RED: one canonical event stream must expose matching conversation, approval/Mission Control, preview, and deployment projections to the web subscriber and desktop cloud controller.
- [x] GREEN: drive both clients from the same typed stream and compare their complete projections, including preview-ready and deployment-stage state.
- [x] Verify: focused integration plus desktop cloud unit suite, UI lint/typecheck, and desktop main typecheck.
- [x] Commit: `test(clients): cover cross-platform builder projections`
...
- 2026-08-13 MAC-6-FIX-5 done — Reconciled the full desktop corpus with operation-scoped typed contained fixtures, retained public/runtime and renderer coverage while retiring provider-private tag expectations, and removed test-network/resource races; exact `test:unit` passed TAP 10/10, unit 3109/3109, and serial integration 131/131, static gates passed, and all four Playwright discovery shards listed 281 tests (71+70+73+67). The binding file list was expanded to the runtime seams required by the failures; no provider call was needed.
- 2026-08-13 MAC-6-FIX-5 review fix — Reused the orchestrator context assembler so resumed transcripts retain untrusted provenance, restored typed queued-message attachment/component lifecycle coverage, and made generated-image attachments explicitly unavailable in contained local mode; focused session 13/13, queue 1/1, and local-agent integration 2/2 passed.
