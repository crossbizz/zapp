...
- [x] **Step 4:** Verify: `pnpm modal:publish --env dev` then a smoke script creates a VM Sandbox from the tag with the pinned SDK/runtime option, `node --version` → `v22.*`, agent healthz responds, and ADR-0007's cgroup create/join/kill/`populated 0` capability probe passes. The smoke must also verify every P0 feature consumed by the runtime (volumes, filesystem snapshots, encrypted tunnels, and readiness probes); unsupported capability is a visible blocker, never a downgrade. Provider connect tokens are not a V2 capability and are replaced by ADR-0023's zapp-owned ingress. Commit: `feat(infra): versioned Modal base images with baked workspace agent`
...
- [x] **4a TDD and implementation evidence.** Lifecycle tests failed first at the
  absent app/provider boundary, then passed with strict create/status/terminate,
  authenticated readiness, requested-to-ready row transitions, tuple idempotency,
  failure compensation, immutable image selection, and exact tags/resources/env.
  The focused and package/static suites were green before the 4a implementation
  commit `feat(sandbox-service): Modal workspace create/status/terminate`.

##### Slice 4b — strict agent proxy, conformance, guarded revisions, and transport

- [x] **4b proxy and unguarded TDD evidence.** Focused tests failed first at absent
  provider operations/routes, then passed for every strict authenticated WS-3 proxy,
  tenant-bound internal route, response schema, cancellation/kill propagation, and
  shared unguarded atomic/search/delete/rename/dev-server case. The locked-image live
  matrix later proved the alias/symlink and unguarded rollback guarantees.
- [x] **4b request-transport RED/GREEN.** Add a focused adapter regression proving an
  allowed 8 MiB request cannot be carried as one `node -e` argument. Stream the exact
...
- [x] **4c TDD and implementation evidence.** Restart recovery failed first at absent
...
**Files:** Create: `src/secrets/injector.ts`, `src/network/profiles.ts`, `test/injection.test.ts`; Modify: `src/app.ts`, `src/routes/workspaces.ts`, `test/integration/{modal-provider,git-clone}.test.ts`, `docs/adr/0024-launch-scoped-workspace-secrets.md`
**Effort:** M

- [x] Binding behavior (ADR-0024): at the service-authenticated app-command launch boundary, sandbox-service calls CP-7 for strict project+environment secret references and injects values into that child process **only**. The agent daemon environment/config receives no values; `ZAPP_SECRET_NAMES` accompanies the child request for name-only diagnostics. The internal route accepts no raw caller `env`. Env allowlist tests assert injected env ⊆ {resolved user secrets, `ZAPP_SECRET_NAMES`, platform runtime vars} and reject `MODAL_*`, `DATABASE_URL`, `SERVICE_TOKEN_*`, `STYTCH_*`, `GRAFANA_*`, `FLEXPRICE_*` (Global Constraint 5). Buffered and streaming stdout/stderr are scrubbed before sandbox-service writes response bytes, including values split across chunks. Network profiles (PRD §18.11) remain strict policy data: `dependency_install` (registries+git hosts+configured integrations), `build_test` (integrations only), `restricted_verification` (deny-all or strict allowlist); the create boundary always emits the resolved defense-in-depth policy record and records that pinned Modal V2 has no provider enforcement primitive rather than claiming a downgrade as containment.
- [x] Commit: `feat(sandbox-service): scoped secret injection, network profiles, output redaction`

### Task WS-12: Preview access tokens + share records

**Files:** Create: `services/sandbox-service/src/preview/{transport,tokens}.ts`, `services/sandbox-service/src/routes/preview.ts`, `services/sandbox-service/test/preview-auth.test.ts`, DB migration, `services/control-api/src/routes/preview.ts`, control-api tests, `apps/web/src/app/preview/[organizationId]/[shareId]/page.tsx`, web e2e; Modify: `packages/db/src/schema/projects.ts`, `packages/db/src/tenant.ts`, DB exports, sandbox provider/app composition, contracts, control-api composition/env/server, `.env.example`, OpenAPI/generated SDK, package manifests/lockfile, WEB-7 and MAC-8 consumer text as required by ADR-0023.
**Effort:** L. **Execution is serial in three slices; one final real-Modal acceptance only.**

- [x] **Slice A — internal transport RED/GREEN:** provision encrypted port 8080; service-authenticated, tenant/project/workspace-scoped HTTP + WebSocket pass-through to the Modal encrypted tunnel; never serialize the provider URL or accept a client-selected upstream. Strip hop-by-hop and zapp auth/session/tenant headers, preserve application headers/cookies only, propagate aborts, and stream without buffering. Fake-SDK tests prove headers/status/stream/upgrade and cross-tenant 404. The final env-gated test proves a locked-image preview-proxy health request through this transport.
- [x] **Slice B — public API RED/GREEN:** remove the stale provider-facing `CloudSandboxProvider.createPreview()` method and associated input type; add tenant-owned durable hashed share rows and strict management create/list/revoke plus two-step session contracts and generated SDK. Keyed create derives a stable DNS-safe share locator and versioned HMAC bearer, stores only its Argon2id hash/key version, and replays the identical response without a plaintext durable secret or generic response cache. The configured `{organizationLocator}-{shareLocator}.PREVIEW_BASE_DOMAIN` origin structurally scopes anonymous lookup. A manually keyed control-api exchange validates the share bearer plus the existing org session when policy requires it and returns only a short-lived hashed bootstrap grant; a manually keyed exact-Origin redemption on the share origin turns that grant into a hashed tenant-bound Redis session and reissues its host-only HttpOnly cookie on replay. Every HTTP stream/WebSocket rechecks the Redis session and expiry/revocation; Redis fanout deletes grants/sessions and closes live traffic, while a five-second per-connection row check bounds revocation to ≤10 s. No list/audit/event or post-redemption response contains a bearer, hash, grant, or Modal URL.
- [x] **Slice C — web RED/GREEN:** the app-host bootstrap reads the URL fragment, authorizes it through the generated control-api SDK, redeems the returned one-time grant against the share's isolated zapp preview origin with credentials enabled, clears fragment/grant memory, and loads that origin so root-relative paths, cookies, service workers, and HMR/WebSockets cannot collide with control-api or another preview. E2E covers org access, anonymous link access, missing/invalid token, exact-Origin CORS, list secrecy, keyed replay/lost responses for both steps, grant expiry/reuse, revoke during HTTP and WebSocket use, two simultaneous preview origins, and no duplicate/raw-provider URL exposure.
- [x] Commit: `feat(sandbox-service): authenticated previews + revocable share records`
...
- 2026-08-09 WS-11 done — ADR-0024 moves decryption to the existing child-only exec boundary, closes raw-env and tenant-scope paths, redacts buffered/streamed output, records keyed network policy before allocation, and passed one locked-image Modal injection/redaction acceptance after two review rounds.
