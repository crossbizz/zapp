...
- [ ] **Slice A — internal transport RED/GREEN:** provision encrypted port 8080; service-authenticated, tenant/project/workspace-scoped HTTP + WebSocket pass-through to the Modal encrypted tunnel; never serialize the provider URL or accept a client-selected upstream. Strip hop-by-hop and zapp auth/session/tenant headers, preserve application headers/cookies only, propagate aborts, and stream without buffering. Fake-SDK tests prove headers/status/stream/upgrade and cross-tenant 404. The final env-gated test proves a locked-image preview-proxy health request through this transport.
