...
- [ ] **ADR-0006 remediation:** failing deterministic race tests pause after opening an in-root parent, swap its pathname to an outside symlink, and prove read/write/list plus buffered/PTy `cwd` execution either use the pinned in-root descriptor or reject without outside side effects. Implement the auditable native helper with Linux `openat2(RESOLVE_BENEATH|RESOLVE_NO_SYMLINKS)` when available and component-wise `openat`/`O_NOFOLLOW` fallback for supported macOS/older Linux; package/build tests fail if the helper is absent or non-executable. Commit: `fix(sandbox): enforce descriptor-relative workspace paths`.
- [ ] **ADR-0007 remediation:** failing containment tests prove detached/`setsid()` descendants cannot survive timeout, disconnect, explicit kill, or shutdown, and that completed execution ownership ends only after the authoritative empty signal. Implement one cgroup-v2 subtree per execution; the native launcher joins and verifies it before user code, termination uses `cgroup.kill`, cleanup waits for `cgroup.events` `populated 0`, and production fails closed when unavailable. macOS uses an injected containment double; no PID/PGID polling fallback. Commit: `fix(sandbox): contain workspace executions with cgroup v2`.
- [ ] **ADR acceptance gate:** run both full workspace-agent suites on current Node and Node 22, WS-1 conformance, frozen install/native-helper mode repair, and uncached repository test/lint/typecheck/build. Then run WS-2's real Modal dev VM smoke before checking WS-3 complete; until the image exists, report this gate as blocked/unverified rather than passed.
...
- 2026-08-05 WS-3 Phase-2 DONE_WITH_CONCERNS — cgroup-v2 per-exec containment implemented with native launcher join verification, fail-closed production behavior, detached-descendant cleanup coverage, and PID-ownership/cgroup-ownership separation. Evidence: uncached repo test/lint/typecheck/build, WS-1 conformance 12/12, frozen install/native-helper checks, then final post-fix workspace-agent current Node 71/71, Node 22 71/71, lint, typecheck, and build; post-run orphan scans were clean. WS-3 remains unchecked because the real Modal dev VM smoke/ADR-0007 provider capability gate is deferred.
- 2026-08-05 WS-3 review fixes verified / smoke pending — independent re-review passed spec and quality after retaining ownership through failed cgroup cleanup and reading metrics from `ZAPP_CGROUP_ROOT`. Final post-fix evidence: focused regressions 9/9, workspace-agent current Node 74/74, Node 22.23.1 74/74, uncached repository test/lint/typecheck/build 17/17, 14/14, 13/13, 8/8, clean diff and orphan scans. WS-3 remains unchecked until WS-2's real Modal dev VM capability smoke passes.
