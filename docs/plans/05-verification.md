...
- 2026-08-10 VF-13 done — Added classified repair loops with exact transient/deterministic budgets, AR-7-scoped builder context, stage-keyed production persistence, exact-parent commit verification, fresh affected-gate checks, and replay-safe escalation; the single capped review found four Important issues and all were fixed, with no interface deviations or real-provider run prescribed.
- 2026-08-11 VF-13-FIX-1 done — Raised only the contradictory real-Git repair-exhaustion integration case from Vitest's 5-second default to a bounded 15 seconds after the clean isolated pre-push gate timed out at 5.13 seconds; production budgets and assertions are unchanged.
