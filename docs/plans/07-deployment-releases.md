...
- [ ] **DEP-12b commit:** `feat: end-to-end release lifecycle live behind /v1 APIs`
- [ ] **Final gate:** run touched-package lint/typecheck/build/tests, the root integration command, one capped review (exit: no unresolved Major structural/correctness/security finding), then exactly one env-gated Fly staging journey; update tracker/log, commit, push, and require exact-SHA GitHub CI + Security green.
