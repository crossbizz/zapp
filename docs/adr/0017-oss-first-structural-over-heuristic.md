...
|---|---|---|
| Inject/rewrite HTML in the preview proxy (WS-10) | a regex/hand HTML scanner | a streaming HTML rewriter — `lol-html` (Cloudflare `HTMLRewriter`) or `parse5`; injection before `</head>` is a solved, tested operation |
...
| Task/plan DAG scheduling (AR-11) | a bespoke scheduler | `graphlib` topological sort (`dagre` is already present for layout) |
...
only on a gap in a structural control or a genuine correctness/security defect. The
