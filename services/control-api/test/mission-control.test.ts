...
describe('GET /v1/runs/:runId/mission-control', () => {
  ...
  it('returns server-computed retry and optional-phase eligibility with stable reasons', async () => {
    ...
    addEvent(seeded, 'phase.created', {
      phaseId: optionalPhaseId, sequence: 2, title: 'Optional', status: 'running',
    }, { phaseId: optionalPhaseId });
    const startedPhase = await seeded.harness.app.inject({
      method: 'GET',
      url: `/v1/runs/${seeded.run.id}/mission-control`,
      headers: seeded.headers,
    });
    expect(startedPhase.statusCode, startedPhase.body).toBe(200);
    expect(startedPhase.json<{ actions: {
      skipOptionalPhases: { phaseId: string; eligible: boolean; reason: string }[];
    } }>().actions.skipOptionalPhases).toContainEqual({
      phaseId: optionalPhaseId,
      eligible: false,
      reason: 'phase_task_started',
    });

    seeded.run.status = 'dispatch_failed';
    const inactive = await seeded.harness.app.inject({
      method: 'GET',
      url: `/v1/runs/${seeded.run.id}/mission-control`,
      headers: seeded.headers,
    });
    expect(inactive.statusCode, inactive.body).toBe(200);
    const inactiveActions = inactive.json<{ actions: {
      retryFailedTasks: { taskId: string; reason: string }[];
      skipOptionalPhases: { phaseId: string; reason: string }[];
    } }>().actions;
    expect(inactiveActions.retryFailedTasks.find(({ taskId }) => taskId === failedTaskId)?.reason)
      .toBe('run_not_active');
    expect(inactiveActions.skipOptionalPhases.find(({ phaseId }) => phaseId === optionalPhaseId)?.reason)
      .toBe('run_not_active');
  });
});
